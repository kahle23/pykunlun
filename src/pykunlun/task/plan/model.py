...
集中承载计划任务子包中"纯数据容器"性质的五个类，字段与各后端实现的五张任务表
（任务/步骤/执行/产物/事件）一一对应（对标 :mod:`pykunlun.ai.ocr.model` 的组织方式）：

  - :class:`PlanTask`     —— 计划任务（核心表：一次具体的计划任务）
...
class PlanTask:
    ...
    字段与存储层的任务表（``plan_task_instance``）一一对应。新建时 ``id`` 留空，由存储回填；
    ``status`` 新建固定为 ``pending``。

    Attributes:
        title: 任务标题。
        goal: 任务目标（喂给编排层的总指令）。
        source_id: 来源标识 id（上层自定义，如 prompt 库/技能侧的内部 id；落库映射列
            ``template_id``）；None = 无来源。
...
class TaskStep:
    ...
    Attributes:
        ...
        max_retries: 最大重试次数（含首次共 ``max_retries + 1`` 次机会）；None 表示
            未指定，落库时继承任务级 ``PlanTask.max_retries``。
