...
class PlanTaskManager:
    ...
    典型用法::
        ...
        from pykunlun.task.plan import PlanTask, PlanTaskManager

        mgr = PlanTaskManager()
        mgr.register('default', SqlitePlanTaskService('/data/agent_task.db'))
        tid = mgr.create_task(PlanTask(title='补全文档', goal='...'))
    ...
    # region ======== 任务 ========
    def create_task(self, task: PlanTask, name: str | None = None) -> int:
        return self.get_service(name).create_task(task)

    def get_task(self, id: int, name: str | None = None) -> PlanTask | None:
