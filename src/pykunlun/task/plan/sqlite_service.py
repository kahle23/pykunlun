...
跨进程可读、重启不丢失；建 ``plan_task_*`` 五张表，仅依赖标准库 ``sqlite3``。
...
JSON 字段（params）以 ``ensure_ascii=False``
...
from .model import PlanTask, TaskRun, TaskStep
...
class SqlitePlanTaskService(PlanTaskService):
    ...
    service_type: ClassVar[str] = "sqlite"
    ...
    def setup(self) -> None:
        """
        幂等建 ``plan_task_*`` 五张表 + 索引，并给老库自动补新增列（幂等可重复执行）。
        """
        with self._conn() as conn:
            self._ddl_all(conn)
            self._migrate(conn)
        log.info("SqlitePlanTaskService 已初始化 5 张 plan_task_* 表 (db=%s)", self._db_path)
    ...
    def _task_row(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        # 物理列名沿用 template_id（表结构不动），领域字段名为 source_id（来源标识）
        row["source_id"] = row.pop("template_id", None)
        return row
    ...
    # region ======== 任务 ========
    def create_task(self, task: PlanTask) -> int:
        now = datetime.now()
        with self._conn() as conn:
            cur = conn.execute(
            ...
            self._event(conn, task_id, "note", f"任务创建: {task.title}")
        task.id = task_id
        return task_id

    def get_task(self, id: int) -> PlanTask | None:
        rows = self._query(f"SELECT * FROM {self._t('plan_task_instance')} WHERE id = ?", (id,))
        return PlanTask.from_dict(self._task_row(rows[0])) if rows else None
