"""
计划任务服务的内置实现（基于 Python 标准库 sqlite3，无第三方依赖）。

对标 :mod:`pykunlun.ai.ocr.rapidocr_engine`：作为计划任务子包内的**轻量本地默认实现**
随 :class:`~pykunlun.task.plan.manager.PlanTaskManager` 开箱即用，提供
:class:`SqlitePlanTaskService`——基于 SQLite 文件的计划任务服务，单文件持久化、
跨进程可读、重启不丢失；建 ``plan_task_*`` 六张表，仅依赖标准库 ``sqlite3``。

适用于无需配置 rdb 的轻量部署、单测、快速起步。需要 MySQL，或想复用已注册的
rdb 实例时，使用上层包的 ``MySqlPlanTaskService``。
...
class SqlitePlanTaskService(PlanTaskService):
    """
    基于 SQLite 的计划任务服务（标准库 sqlite3，单文件持久化）。
    ...
    旧版（表名 ``ai_task_*`` 时期）创建的 SQLite 库不做数据迁移：setup 会另建
    ``plan_task_*`` 新表从头开始，旧表保留在库文件中成为孤儿数据。

    Args:
        db_path: SQLite 数据库文件路径。**不支持** ``':memory:'``——connect-per-call
            下每次连接彼此隔离、数据不跨连接留存；测试请使用临时文件。
        table_prefix: 表名前缀（默认空）。表名固定 ``plan_task_*``，前缀用于同一库内
            隔离多套任务表。
    """
    ...
    def __init__(self, db_path: str, table_prefix: str = "") -> None:
        if db_path == ":memory:":
            raise ValueError(
                "SqlitePlanTaskService 不支持 ':memory:'（connect-per-call 下数据不跨连接留存）；"
    ...
    @staticmethod
    def _q(conn: sqlite3.Connection, sql: str, params: tuple[Any, ...] = ()) -> list[dict[str, Any]]:
        """
    ...
    def _event(
        ...
        """
        conn.execute(
            f"INSERT INTO {self._t('plan_task_event')} "
    ...
    def setup(self) -> None:
        """
        幂等建 ``plan_task_*`` 六张表 + 索引，并给老库自动补新增列（幂等可重复执行）。
        """
        with self._conn() as conn:
            self._ddl_all(conn)
            self._migrate(conn)
        log.info("SqlitePlanTaskService 已初始化 6 张 plan_task_* 表 (db=%s)", self._db_path)

    def _migrate(self, conn: sqlite3.Connection) -> None:
        """轻量补列迁移：CREATE TABLE IF NOT EXISTS 不会给已存在的表补新列，
        此处按 ``PRAGMA table_info`` 探测缺列并 ALTER（0.0.5 起的机制，新增列在此登记）。"""
        t = self._t("plan_task_step")
    ...
    def _ddl_all(self, conn: sqlite3.Connection) -> None:
        p = self._prefix
        conn.execute(f"""CREATE TABLE IF NOT EXISTS {p}plan_task_template (
        ...
        conn.execute(f"""CREATE TABLE IF NOT EXISTS {p}plan_task_instance (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            template_id INTEGER,
        ...
        conn.execute(f"""CREATE TABLE IF NOT EXISTS {p}plan_task_step (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        conn.execute(f"""CREATE TABLE IF NOT EXISTS {p}plan_task_run (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            task_id INTEGER NOT NULL,
        ...
        conn.execute(f"""CREATE TABLE IF NOT EXISTS {p}plan_task_artifact (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            task_id INTEGER NOT NULL,
            step_id INTEGER,
        ...
        conn.execute(f"""CREATE TABLE IF NOT EXISTS {p}plan_task_event (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            task_id INTEGER NOT NULL,
            step_id INTEGER,
            run_id INTEGER,
        ...
        indexes = [
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_inst_status ON {p}plan_task_instance (status)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_inst_zombie ON {p}plan_task_instance (status, heartbeat_at)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_inst_template ON {p}plan_task_instance (template_id)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_inst_parent ON {p}plan_task_instance (parent_task_id)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_step_status ON {p}plan_task_step (task_id, status)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_run_step ON {p}plan_task_run (step_id)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_run_task ON {p}plan_task_run (task_id)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_run_status ON {p}plan_task_run (status)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_art_task ON {p}plan_task_artifact (task_id)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_art_step ON {p}plan_task_artifact (step_id)",
            f"CREATE INDEX IF NOT EXISTS idx_{p}plan_task_event_task ON {p}plan_task_event (task_id, id)",
        ]
        for stmt in indexes:
            conn.execute(stmt)
    ...
    def _fetch_task(self, conn: sqlite3.Connection, task_id: int) -> dict[str, Any] | None:
        rows = self._q(conn, f"SELECT * FROM {self._t('plan_task_instance')} WHERE id = ?", (task_id,))
    ...
    def create_task(self, inst: TaskInstance) -> int:
        ...
        with self._conn() as conn:
            cur = conn.execute(
                f"INSERT INTO {self._t('plan_task_instance')} "
    ...
    def get_task(self, id: int) -> TaskInstance | None:
        rows = self._query(f"SELECT * FROM {self._t('plan_task_instance')} WHERE id = ?", (id,))
    ...
    def update_task(self, id: int, fields: dict[str, Any]) -> bool:
        ...
        params.append(id)
        sql = f"UPDATE {self._t('plan_task_instance')} SET {', '.join(set_parts)} WHERE id = ?"
    ...
    def pause(self, id: int) -> bool:
        ...
        affected = self._execute(
            f"UPDATE {self._t('plan_task_instance')} "
    ...
    def cancel(self, id: int, reason: str = "") -> bool:
        ...
        with self._conn() as conn:
            ...
            steps = self._q(
                conn, f"SELECT * FROM {self._t('plan_task_step')} WHERE task_id = ? AND status = ?", (id, "running")
            )
            conn.execute(
                f"UPDATE {self._t('plan_task_run')} SET status = ?, finished_at = ? WHERE task_id = ? AND status = ?",
                ("cancelled", _iso(now), id, "running"),
            )
            for s in steps:
                conn.execute(
                    f"UPDATE {self._t('plan_task_step')} "
                    f"SET status = ?, finished_at = ?, updated_at = ? WHERE id = ? AND status = ?",
                    ("failed", _iso(now), _iso(now), s["id"], "running"),
                )
                self._event(conn, id, "state_change", f"step {s['id']}: running → failed (cancelled)", step_id=s["id"])
            conn.execute(
                f"UPDATE {self._t('plan_task_instance')} SET status = ?, finished_at = ?, updated_at = ? WHERE id = ?",
    ...
    def _validate_step(self, step: TaskStep) -> None:
        ...
        if step.step_type not in VALID_STEP_TYPES:
            raise ValueError(f"非法的 step_type: {step.step_type!r}（合法值: {sorted(VALID_STEP_TYPES)}）")
        if step.depends_on is not None:
            deps = step.depends_on
            if not isinstance(deps, list) or any(isinstance(v, bool) or not isinstance(v, int) for v in deps):
                raise ValueError(f"步骤 {step.name!r} 的 depends_on 须为 int 列表（依赖步骤的 seq）")
    ...
    def _check_deps_vs_seq(self, step: TaskStep, seq: int) -> None:
        ...
        if bad:
            raise ValueError(
                f"步骤 {step.name!r} 的 depends_on 只能引用同任务更早的 seq （非法: {bad}，自身 seq={seq}）"
            )
    ...
    def _resolve_step_defaults(self, conn: sqlite3.Connection, step: TaskStep) -> tuple[int, int]:
        ...
        """
        rows = self._q(
            conn, f"SELECT MAX(seq) AS mx FROM {self._t('plan_task_step')} WHERE task_id = ?", (step.task_id,)
        )
    ...
    def _insert_step(self, conn: sqlite3.Connection, step: TaskStep, seq: int, max_retries: int) -> int:
        now = _iso(datetime.now())
        cur = conn.execute(
            f"INSERT INTO {self._t('plan_task_step')} "
    ...
    def add_step(self, step: TaskStep) -> int:
        ...
        with self._conn() as conn:
            ...
            self._event(conn, step.task_id, "note", f"step added: seq={seq} {step.name} (step_id={step_id})")
        step.seq = seq
        step.max_retries = mr
        step.id = step_id
        return step_id
    ...
    def get_step(self, id: int) -> TaskStep | None:
        rows = self._query(f"SELECT * FROM {self._t('plan_task_step')} WHERE id = ?", (id,))
        return TaskStep.from_dict(rows[0]) if rows else None

    def list_steps(self, task_id: int) -> list[dict[str, Any]]:
        return self._query(f"SELECT * FROM {self._t('plan_task_step')} WHERE task_id = ? ORDER BY seq", (task_id,))

    def skip_step(self, id: int, reason: str = "") -> bool:
        t_task = self._t("plan_task_instance")
        t_step = self._t("plan_task_step")
        now = _iso(datetime.now())
        with self._conn() as conn:
            ...
            affected = conn.execute(
                f"UPDATE {t_step} SET status = ?, finished_at = ?, updated_at = ? WHERE id = ? AND status = ?",
            ...
            cnt = self._q(
                conn,
                f"SELECT status, COUNT(*) AS n FROM {t_step} WHERE task_id = ? AND status IN (?,?,?) GROUP BY status",
            ...
            if not counts.get("pending") and not counts.get("running") and not counts.get("failed"):
                ...
                self._event(conn, rows[0]["task_id"], "state_change", "task: running → completed (剩余步骤全部跳过)")
        return True

    def retry_step(self, id: int, force: bool = False) -> bool:
        now = datetime.now()
        with self._conn() as conn:
            rows = self._q(conn, f"SELECT * FROM {self._t('plan_task_step')} WHERE id = ?", (id,))
            if not rows:
            ...
            if step["status"] not in allowed:
                log.warning(
                    "步骤 id=%s 状态为 %s，仅 failed/skipped 可 retry%s",
                    id,
                    step["status"],
                    "（force 可加 succeeded）" if not force else "",
                )
                return False
            clear_summary = force and step["status"] == "succeeded"
            sets = (
                "status = ?, max_retries = max_retries + 1, updated_at = ?"
                if not clear_summary
                else "status = ?, max_retries = max_retries + 1, result_summary = NULL, "
                "finished_at = NULL, updated_at = ?"
            )
            conn.execute(f"UPDATE {self._t('plan_task_step')} SET {sets} WHERE id = ?", ("pending", _iso(now), id))
            tag = "manual retry, budget+1" + (", force" if force else "")
            self._event(
                conn, step["task_id"], "state_change", f"step {id}: {step['status']} → pending ({tag})", step_id=id
            )
    ...
    def claim_next_step(
        ...
        with self._conn() as conn:
            ...
            for _ in range(3):
                cands = self._q(
                    conn, f"SELECT * FROM {t_step} WHERE task_id = ? AND status = ? ORDER BY seq", (task_id, "pending")
                )
                if not cands:
                    return None
                ...
                if not ignore_deps:
                    all_steps = self._q(
                        conn, f"SELECT seq, status, depends_on FROM {t_step} WHERE task_id = ?", (task_id,)
                    )
                    dep_aware = any(parse_depends_on(s["depends_on"]) for s in all_steps)
                    if dep_aware:
                        status_by_seq = {s["seq"]: s["status"] for s in all_steps}
                        cand = next(
                            (c for c in cands if deps_satisfied(parse_depends_on(c["depends_on"]), status_by_seq)), None
                        )
                        if cand is None:
                            log.info("任务 id=%s 的 pending 步骤依赖均未就绪，暂无可认领", task_id)
                            return None
                if cand is None:
                    cand = cands[0]
                affected = conn.execute(
                    ...
                    f"updated_at = ? WHERE id = ? AND status = ?",
                    ("running", _iso(now), _iso(now), cand["id"], "pending"),
                ).rowcount
                if not affected:
                    continue
                ...
                if task["status"] == "pending":
                    ...
                    self._event(conn, task_id, "state_change", "task: pending → running (first claim)")
                else:
                    self._touch_heartbeat(conn, task_id)
                # 续跑上下文包：任务 + 步骤 + run_id + 前序成功步骤摘要
                ctx = self._q(
                    conn,
                    f"SELECT seq, name, result_summary FROM {t_step} WHERE task_id = ? AND status = ? ORDER BY seq",
                    (task_id, "succeeded"),
                )
                trow = self._task_row(self._q(conn, f"SELECT * FROM {t_task} WHERE id = ?", (task_id,))[0])
                srow = self._q(conn, f"SELECT * FROM {t_step} WHERE id = ?", (cand["id"],))[0]
                package: dict[str, Any] = {"task": trow, "step": dict(srow), "context": ctx}
                cur = conn.execute(
                    f"INSERT INTO {self._t('plan_task_run')} "
                    f"(task_id, step_id, session_id, agent_name, status, input_snapshot, "
                    f"started_at) VALUES (?,?,?,?,?,?,?)",
                    (task_id, cand["id"], session_id, agent_name, "running", _jdump(package), _iso(now)),
                )
                ...
                self._event(
                    ...
                    task_id,
                    "state_change",
                    f"step {cand['id']}: pending → running (claim by {session_id or 'anonymous'})",
                    step_id=cand["id"],
                    run_id=run_id,
                )
    ...
    def finish_run(
        ...
        with self._conn() as conn:
            ...
            self._event(
                ...
                run["task_id"],
                "state_change",
                f"run {run_id}: running → succeeded",
                step_id=run["step_id"],
                run_id=run_id,
            )
            self._event(
                ...
                run["task_id"],
                "state_change",
                f"step {run['step_id']}: running → succeeded",
                step_id=run["step_id"],
            )
        ...
        with self._conn() as conn:
            ...
            if disposition == "pending":
                ...
                self._event(
                    ...
                    run["task_id"],
                    "state_change",
                    f"step {step['id']}: running → pending (retry {step['retry_count'] + 1}/{mr})",
                    step_id=step["id"],
                )
                self._touch_heartbeat(conn, run["task_id"])
            else:
                ...
                conn.execute(
                    f"UPDATE {t_task} SET status = ?, finished_at = ?, updated_at = ? WHERE id = ? AND status = ?",
                    ("failed", _iso(now), _iso(now), run["task_id"], "running"),
                )
                self._event(conn, run["task_id"], "state_change", "task: running → failed (步骤预算耗尽)")
            return "retried" if disposition == "pending" else "step_failed"

    def get_run(self, run_id: int) -> TaskRun | None:
        rows = self._query(f"SELECT * FROM {self._t('plan_task_run')} WHERE id = ?", (run_id,))
        return TaskRun.from_dict(rows[0]) if rows else None

    def list_runs(self, step_id: int) -> list[dict[str, Any]]:
        return self._query(f"SELECT * FROM {self._t('plan_task_run')} WHERE step_id = ? ORDER BY id", (step_id,))

    def list_task_runs(self, task_id: int) -> list[dict[str, Any]]:
        return self._query(f"SELECT * FROM {self._t('plan_task_run')} WHERE task_id = ? ORDER BY id", (task_id,))

    def release_run(self, run_id: int, reason: str = "") -> str:
        now = datetime.now()
        with self._conn() as conn:
            runs = self._q(conn, f"SELECT * FROM {self._t('plan_task_run')} WHERE id = ?", (run_id,))
            if not runs:
            ...
            if run["status"] != "running":
                ...
                return ""
            conn.execute(
                f"UPDATE {self._t('plan_task_run')} SET status = ?, error_msg = ?, finished_at = ? "
                f"WHERE id = ? AND status = ?",
                ("cancelled", "released" + (f": {reason}" if reason else ""), _iso(now), run_id, "running"),
            )
            self._event(
                conn,
                run["task_id"],
                "state_change",
                f"run {run_id}: running → cancelled (release{': ' + reason if reason else ''})",
                step_id=run["step_id"],
                run_id=run_id,
            )
            # 步骤还回队列：不加 retry_count（释放 ≠ 失败，不消耗重试预算）
            affected = conn.execute(
                f"UPDATE {self._t('plan_task_step')} SET status = ?, updated_at = ? WHERE id = ? AND status = ?",
                ("pending", _iso(now), run["step_id"], "running"),
            ).rowcount
            if affected:
                self._event(
                    conn,
    ...
    # region ======== 恢复（sweep） ========
    def _reap_step(
        self, conn: sqlite3.Connection, step: dict[str, Any], reason: str, now: datetime
    ) -> list[dict[str, Any]]:
        ...
        runs = self._q(conn, f"SELECT * FROM {t_run} WHERE step_id = ? AND status = ?", (step["id"], "running"))
        for r in runs:
            conn.execute(
                f"UPDATE {t_run} SET status = ?, error_msg = ?, finished_at = ? WHERE id = ? AND status = ?",
        ...
        if disposition == "pending":
            ...
            out.append(
                {
                    ...
                    "run_id": None,
                    "action": "step_retry",
                    "detail": f"retry {step['retry_count'] + 1}/{mr}",
                }
            )
            self._event(
                conn,
                step["task_id"],
                "state_change",
                f"step {step['id']}: running → pending (sweep retry)",
                step_id=step["id"],
            )
        else:
            conn.execute(
            ...
            self._event(conn, step["task_id"], "state_change", "task: running → failed (sweep, 步骤预算耗尽)")
        return out

    def _reap_task_timeout(self, conn: sqlite3.Connection, task: dict[str, Any], now: datetime) -> list[dict[str, Any]]:
        ...
        steps = self._q(conn, f"SELECT * FROM {t_step} WHERE task_id = ? AND status = ?", (task["id"], "running"))
        for s in steps:
            runs = self._q(conn, f"SELECT * FROM {t_run} WHERE step_id = ? AND status = ?", (s["id"], "running"))
            for r in runs:
                conn.execute(
                    f"UPDATE {t_run} SET status = ?, error_msg = ?, finished_at = ? WHERE id = ? AND status = ?",
                    ("timeout", reason, _iso(now), r["id"], "running"),
            ...
            out.append(
                {"task_id": task["id"], "step_id": s["id"], "run_id": None, "action": "step_failed", "detail": reason}
            )
            self._event(
                conn, task["id"], "state_change", f"step {s['id']}: running → failed ({reason})", step_id=s["id"]
            )
        conn.execute(
            f"UPDATE {t_task} SET status = ?, finished_at = ?, updated_at = ? WHERE id = ? AND status = ?",
            ("failed", _iso(now), _iso(now), task["id"], "running"),
        )
        out.append({"task_id": task["id"], "step_id": None, "run_id": None, "action": "task_failed", "detail": reason})
        self._event(conn, task["id"], "state_change", f"task {task['id']}: running → failed (任务总超时)")
        return out
    ...
    def sweep(self, heartbeat_timeout_sec: int | None = None) -> list[dict[str, Any]]:
        ...
        with self._conn() as conn:
            ...
            timed_out = self._q(
                conn, f"SELECT * FROM {t_task} WHERE status = ? AND timeout_sec IS NOT NULL", ("running",)
            )
            for t in timed_out:
                if not t["started_at"]:
                    continue
            ...
            running_tasks = self._q(conn, f"SELECT * FROM {t_task} WHERE status = ?", ("running",))
            for t in running_tasks:
                ...
                timeout = (
                    ...
                    if t["heartbeat_timeout_sec"] is not None
                    else 1800
                )
                ...
                zombie_steps = self._q(
                    conn, f"SELECT * FROM {t_step} WHERE task_id = ? AND status = ?", (t["id"], "running")
                )
            ...
            for s in candidates:
                ...
                live = self._q(
                    conn,
                    f"SELECT * FROM {t_run} WHERE step_id = ? AND status = ? "
    ...
    def verify_task(self, task_id: int, fix: bool = False) -> list[dict[str, Any]]:
        ...
        t_step = self._t("plan_task_step")
        t_run = self._t("plan_task_run")
        t_event = self._t("plan_task_event")
        now = datetime.now()
        ...
        with self._conn() as conn:
            ...
            if task is None:
                raise ValueError(f"任务不存在: id={task_id}")
            steps = self._q(conn, f"SELECT * FROM {t_step} WHERE task_id = ? ORDER BY seq", (task_id,))
            runs = self._q(conn, f"SELECT * FROM {t_run} WHERE task_id = ?", (task_id,))
            events = self._q(conn, f"SELECT message FROM {t_event} WHERE task_id = ?", (task_id,))
            ...
            def has_event(sid: int, fragment: str) -> bool:
                return any(m.startswith(f"step {sid}: ") and fragment in m for m in event_msgs)
            ...
            for s in steps:
                ...
                if s["status"] == "succeeded":
                    ...
                    if fix:
                        ...
                        if live:
                            ...
                            self._event(
                                ...
                                task_id,
                                "state_change",
                                f"step {sid}: succeeded → running (verify fix, 接管活 run)",
                                step_id=sid,
                            )
                        else:
                            conn.execute(
                            ...
                            self._event(
                                ...
                                task_id,
                                "state_change",
                                f"step {sid}: succeeded → pending (verify fix, 清假摘要)",
                                step_id=sid,
                            )
                        finding["fixed"] = True
                    ...
                    if not running_runs(sid):
                        ...
                        if fix:
                            conn.execute(
                                f"UPDATE {t_step} SET status = ?, updated_at = ? WHERE id = ? AND status = ?",
                            ...
                            self._event(
                                ...
                                task_id,
                                "state_change",
                                f"step {sid}: running → pending (verify fix, 清无支撑的 running)",
                                step_id=sid,
                            )
                            finding["fixed"] = True
                        ...
                        findings.append(
                            {
                                ...
                                "kind": "step",
                                "id": sid,
                                "detail": "skipped 但无 skip 事件（来源不明）",
                                "fixed": None,
                            }
                        )
            ...
            for r in runs:
                ...
                finding = {
                    ...
                    "kind": "run",
                    "id": r["id"],
                    "detail": f"run 仍在 running，但所属步骤状态为 {st['status'] if st else '不存在'}",
                    "fixed": None,
                }
                findings.append(finding)
                if fix:
                    ...
                    findings.append(
                        {
                            ...
                            "kind": "task",
                            "id": task_id,
                            "detail": f"{n} 个步骤共享同一 result_summary（伪造指纹）：{summ[:80]}",
                            "fixed": None,
                        }
                    )
            ...
            if task["status"] == "completed":
                ...
                if bad:
                    finding = {
                        ...
                        "kind": "task",
                        "id": task_id,
                        "detail": f"task completed 但存在未终态步骤: {bad}",
                        "fixed": None,
                    }
                    findings.append(finding)
                    if fix:
                        ...
                        self._event(
                            conn, task_id, "state_change", "task: completed → running (verify fix, 收口失真回退)"
                        )
                        finding["fixed"] = True
            if fix and any(f["fixed"] for f in findings):
                n_fixed = sum(1 for f in findings if f["fixed"])
                self._event(conn, task_id, "note", f"verify fix: 修复 {n_fixed} 处不一致", level="warn")
                self._touch_heartbeat(conn, task_id)
        if findings:
    ...
    def add_artifact(
        ...
        with self._conn() as conn:
            cur = conn.execute(
                f"INSERT INTO {self._t('plan_task_artifact')} "
            ...
            self._event(conn, task_id, "artifact", f"产物登记: {path} ({art_type})", step_id=step_id)
        return art_id

    def list_artifacts(self, task_id: int) -> list[dict[str, Any]]:
        return self._query(f"SELECT * FROM {self._t('plan_task_artifact')} WHERE task_id = ? ORDER BY id", (task_id,))
    ...
    def add_event(
        ...
        if event_type not in VALID_EVENT_TYPES:
            raise ValueError(f"非法的 event_type: {event_type!r}（合法值: {sorted(VALID_EVENT_TYPES)}）")
    ...
    def create_template(self, t: TaskTemplate) -> int:
        now = datetime.now()
        with self._conn() as conn:
            dup = self._q(conn, f"SELECT id FROM {self._t('plan_task_template')} WHERE name = ?", (t.name,))
            if dup:
                raise ValueError(f"模板名已存在: {t.name!r} (id={dup[0]['id']})")
            cur = conn.execute(
                f"INSERT INTO {self._t('plan_task_template')} "
                ...
                (
                    ...
                    t.skill_ref,
                    t.description,
                    _jdump(t.default_params),
                    _jdump(t.step_blueprint),
                    _iso(now),
                    _iso(now),
                ),
            )
    ...
    def get_template_by_name(self, template_name: str) -> TaskTemplate | None:
        rows = self._query(f"SELECT * FROM {self._t('plan_task_template')} WHERE name = ?", (template_name,))
    ...
    def list_templates(self, limit: int = 50) -> list[dict[str, Any]]:
        rows = self._query(f"SELECT * FROM {self._t('plan_task_template')} ORDER BY id DESC LIMIT ?", (limit,))
