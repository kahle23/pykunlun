...
def test_version_flag_prints_single_line(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]):
...
def test_version_short_flag(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]):
    # -v 短形态与 --version 等价
...
def test_version_flag_wins_over_command(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]):
...
def test_unknown_command_still_errors(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]):
...
def test_unknown_command_hint_neutral_when_no_main_entry(
...
def test_hint_uses_python_m_when_caller_has_main_entry(monkeypatch: pytest.MonkeyPatch):
    # 调用方包真实提供 __main__ 子模块（pytest 自带 pytest/__main__.py）时保持 python -m 提示
    def fake_get_pkg(skip_packages: list[str] | None = None) -> str:
        return 'pytest'
    monkeypatch.setattr(pkginfo, 'get_caller_top_package_name', fake_get_pkg)
    assert _format_help_hint('help') == "使用 'python -m pytest help' 查看可用命令"


def test_hint_neutral_when_caller_has_no_main_entry(monkeypatch: pytest.MonkeyPatch):
    def fake_get_pkg(skip_packages: list[str] | None = None) -> str:
        return 'tests'
    monkeypatch.setattr(pkginfo, 'get_caller_top_package_name', fake_get_pkg)
