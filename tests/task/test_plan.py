...
@pytest.fixture
def store(store_path: str) -> SqlitePlanTaskService:
...
@pytest.fixture
def mgr(store: SqlitePlanTaskService) -> PlanTaskManager:
...
def _mk_task(store: SqlitePlanTaskService, title: str = '示例任务', max_retries: int = 1,
             heartbeat_timeout_sec: int = 1800, timeout_sec: int | None = None) -> int:
    """建一个任务并返回 id。"""
    return store.create_task(PlanTask(
        title=title, goal='完成示例目标', max_retries=max_retries,
        heartbeat_timeout_sec=heartbeat_timeout_sec, timeout_sec=timeout_sec))


def _mk_steps(store: SqlitePlanTaskService, task_id: int,
              names: tuple[str, ...] = ('第一步', '第二步', '第三步')) -> list[int]:
...
def _task(store: SqlitePlanTaskService, tid: int) -> PlanTask:
    """取任务并断言存在（收窄 get_task 的 Optional，缺行时让测试显式失败）。"""
    t = store.get_task(tid)
    assert t is not None
    return t


def _step(store: SqlitePlanTaskService, sid: int) -> TaskStep:
    """取步骤并断言存在（收窄 get_step 的 Optional）。"""
    s = store.get_step(sid)
    assert s is not None
    return s


def _run(store: SqlitePlanTaskService, rid: int) -> TaskRun:
    """取 run 并断言存在（收窄 get_run 的 Optional）。"""
    r = store.get_run(rid)
...
def test_task_illegal_transitions(old: str, new: str):
...
@pytest.mark.parametrize('old,new', [(o, n) for o, targets in STEP_TRANSITIONS.items()
                                     for n in targets])
def test_step_legal_transitions(old: str, new: str):
...
def test_step_illegal_transitions(old: str, new: str):
    with pytest.raises(ValueError, match='非法|未知'):
        assert_step_transition(old, new)
...
def test_plan_task_dict_roundtrip():
    ...
    r = PlanTask.from_dict({**d, 'unknown_field': 'x'})  # 未知键被忽略
...
def test_manager_forward(mgr: PlanTaskManager, store: SqlitePlanTaskService):
    """Manager 转发到默认实例（含 name 参数插位）。"""
    tid = mgr.create_task(PlanTask(title='T', goal='G'))
...
def test_init_is_idempotent(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    store.setup()
    store.setup()
...
def test_create_and_get_task(store: SqlitePlanTaskService):
    tid = store.create_task(PlanTask(
        title='补文档', goal='补全 README', params={'repo': 'x'}, created_by='kahle', source_id=7))
    assert tid >= 1
    t = _task(store, tid)
    assert t.status == 'pending'
    assert t.params == {'repo': 'x'}   # JSON 列往返
    assert t.created_by == 'kahle'
    assert t.source_id == 7            # source_id ↔ 物理列 template_id 往返
    assert store.get_task(99999) is None


def test_list_tasks_progress(store: SqlitePlanTaskService):
    tid = _mk_task(store)
...
def test_update_task_whitelist(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    assert store.update_task(tid, {'title': '新标题', 'params': {'k': 'v'}}) is True
    assert _task(store, tid).title == '新标题'
    assert _task(store, tid).params == {'k': 'v'}
    assert store.update_task(tid, {'id': 999, 'status': 'completed'}) is False  # 白名单外
    assert _task(store, tid).status == 'pending'


def test_add_step_seq_and_inheritance(store: SqlitePlanTaskService):
    ...
    assert _step(store, s1).seq == 1
    assert _step(store, s2).seq == 2
    assert _step(store, s1).max_retries == 3   # 继承任务级预算
...
def test_add_steps_batch(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    n = store.add_steps([
...
def test_claim_returns_context_package(store: SqlitePlanTaskService):
    ...
    assert pkg['run_id'] >= 1
    # 任务心跳已刷
    assert _task(store, tid).heartbeat_at is not None
    # input_snapshot 已存续跑上下文
    run = _run(store, pkg['run_id'])
    assert run.session_id == 'sess-1' and run.agent_name == 'zcode'
    assert run.input_snapshot and 'step' in run.input_snapshot


def test_claim_context_carries_prior_summaries(store: SqlitePlanTaskService):
    tid = _mk_task(store)
...
def test_claim_no_pending_returns_none(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    ...
    assert _task(store, tid).status == 'completed'   # 末步自动收口


def test_finish_defaults_summary_from_output(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    _mk_steps(store, tid, names=('one',))
    p = store.claim_next_step(tid)
    assert p is not None
    ...
    step = _step(store, p['step']['id'])
    assert step.result_summary == 'x' * 2000           # 缺省截取 output 前 2000 字
    run = _run(store, p['run_id'])
    assert run.status == 'succeeded' and run.output == long_output


def test_finish_terminal_run_returns_false(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    _mk_steps(store, tid, names=('one',))
    p = store.claim_next_step(tid)
    assert p is not None
...
def test_finish_records_token_usage(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    _mk_steps(store, tid, names=('one',))
    p = store.claim_next_step(tid)
    assert p is not None
    ...
    assert _run(store, p['run_id']).token_usage == 12345


def test_finish_without_token_usage_keeps_null(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    _mk_steps(store, tid, names=('one',))
    p = store.claim_next_step(tid)
    assert p is not None
    store.finish_run(p['run_id'], output='ok', summary='完成')
    assert _run(store, p['run_id']).token_usage is None


def test_fail_within_budget_retries(store: SqlitePlanTaskService):
    tid = _mk_task(store, max_retries=1)
    sids = _mk_steps(store, tid, names=('one', 'two'))
    p1 = store.claim_next_step(tid)
    assert p1 is not None
    assert store.fail_run(p1['run_id'], '第一轮失败') == 'retried'
    step = _step(store, sids[0])
    assert step.status == 'pending' and step.retry_count == 1
    assert _task(store, tid).status == 'running'     # 任务不变
...
def test_fail_exhausted_fails_task(store: SqlitePlanTaskService):
    tid = _mk_task(store, max_retries=1)
    sids = _mk_steps(store, tid, names=('one', 'two'))
    p1 = store.claim_next_step(tid)
    assert p1 is not None
    ...
    assert _step(store, sids[0]).status == 'failed'
    assert _task(store, tid).status == 'failed'      # 预算耗尽，任务连带失败
    assert store.claim_next_step(tid) is None          # failed 任务不再派发


def test_fail_terminal_run_returns_empty(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    _mk_steps(store, tid, names=('one',))
    p = store.claim_next_step(tid)
    assert p is not None
...
def test_skip_pending_step(store: SqlitePlanTaskService):
    ...
    assert _step(store, sids[0]).status == 'skipped'
    assert _task(store, tid).status == 'pending'     # 任务状态不变
    p = store.claim_next_step(tid)
    assert p is not None
...
def test_skip_non_pending_fails(store: SqlitePlanTaskService):
    tid = _mk_task(store)
...
def test_skip_last_remaining_step_completes_task(store: SqlitePlanTaskService):
    ...
    assert _task(store, tid).status == 'running'     # two 还 pending，未收口
    assert store.skip_step(sids[1], reason='不需要') is True
    assert _task(store, tid).status == 'completed'   # 跳过最后剩余步骤 → 收口


def test_finish_completes_when_rest_skipped(store: SqlitePlanTaskService):
    ...
    assert p is not None
    store.finish_run(p['run_id'], output='ok', summary='一完成')
    assert _task(store, tid).status == 'completed'


def test_skip_all_steps_keeps_task_pending(store: SqlitePlanTaskService):
...
def test_retry_step_revives_failed(store: SqlitePlanTaskService):
    tid = _mk_task(store, max_retries=0)
    sids = _mk_steps(store, tid, names=('one',))
    p = store.claim_next_step(tid)
    assert p is not None
    store.fail_run(p['run_id'], 'boom')                # 预算 0，直接终败
    assert _task(store, tid).status == 'failed'
    assert store.retry_step(sids[0]) is True
    step = _step(store, sids[0])
    assert step.status == 'pending' and step.max_retries == 1   # 预算 +1
    assert _task(store, tid).status == 'running'     # 任务复活
    p2 = store.claim_next_step(tid)
    assert p2 is not None
    assert p2['step']['id'] == sids[0]


def test_retry_only_failed_or_skipped(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    sids = _mk_steps(store, tid, names=('one',))
    assert store.retry_step(sids[0]) is False          # pending 不可 retry


def test_pause_resume(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    _mk_steps(store, tid, names=('one',))
    p = store.claim_next_step(tid)
    assert p is not None
    assert store.pause(tid) is True
    assert _task(store, tid).status == 'paused'
    ...
    assert _task(store, tid).status == 'running'
    # 恢复后继续认领剩余步骤
    store.finish_run(p['run_id'], output='ok')
    assert _task(store, tid).status == 'completed'


def test_cancel_connected_effects(store: SqlitePlanTaskService):
    tid = _mk_task(store)
    sids = _mk_steps(store, tid, names=('one', 'two'))
    p = store.claim_next_step(tid)
    assert p is not None
    ...
    assert _step(store, sids[0]).status == 'failed'          # running 步骤连带 failed
    assert _run(store, p['run_id']).status == 'cancelled'    # running run 置 cancelled
    assert _step(store, sids[1]).status == 'pending'         # 未动
    assert store.cancel(tid) is False                          # 已终态不可再 cancel


def test_cancel_pending_task(store: SqlitePlanTaskService):
    tid = _mk_task(store)
...
def test_sweep_task_level_heartbeat_timeout(store: SqlitePlanTaskService):
    ...
    assert _run(store, p['run_id']).status == 'timeout'
    assert _run(store, p['run_id']).error_msg == 'heartbeat timeout'
    assert _step(store, p['step']['id']).status == 'pending'      # 预算未耗尽 → 回 pending
    assert _task(store, tid).status == 'running'                  # 任务保持 running
...
def test_sweep_task_level_budget_exhausted(store: SqlitePlanTaskService):
...
def test_sweep_step_level_timeout(store: SqlitePlanTaskService):
    ...
    results = store.sweep()
    assert any(r['action'] == 'run_timeout' and r['run_id'] == p['run_id'] for r in results)
    assert _step(store, sid).status == 'pending'   # 预算未耗尽回 pending


def test_sweep_healthy_task_untouched(store: SqlitePlanTaskService):
...
def test_sweep_idempotent_on_zombie(store: SqlitePlanTaskService):
...
def test_sweep_task_total_timeout(store: SqlitePlanTaskService):
    ...
    assert _run(store, p['run_id']).status == 'timeout'
    assert _run(store, p['run_id']).error_msg == 'task total timeout'
    assert _step(store, sids[0]).status == 'failed'  # 预算 5 也直接终败
    assert _task(store, tid).status == 'failed'
    assert store.sweep() == []                          # 幂等


def test_sweep_task_total_timeout_revive_by_retry(store: SqlitePlanTaskService):
    ...
    store.sweep()
    assert store.retry_step(sids[0]) is True
    assert _step(store, sids[0]).status == 'pending'
    assert _task(store, tid).status == 'running'


def test_sweep_total_timeout_ignores_unset(store: SqlitePlanTaskService):
...
def test_event_validates_types(store: SqlitePlanTaskService):
    tid = _mk_task(store)
...
def test_artifacts(store: SqlitePlanTaskService):
    tid = _mk_task(store)
...
def test_run_task_bash_success(mgr: PlanTaskManager, store: SqlitePlanTaskService):
    tid = _mk_task(store)
...
def test_run_task_bash_fail_budget_flow(mgr: PlanTaskManager, store: SqlitePlanTaskService):
    tid = _mk_task(store, max_retries=1)
...
def test_run_task_timeout_kills_tree(mgr: PlanTaskManager, store: SqlitePlanTaskService):
    tid = _mk_task(store)
...
def test_run_task_agent_release_and_stop(mgr: PlanTaskManager, store: SqlitePlanTaskService):
    tid = _mk_task(store)
...
def test_run_task_empty_instruction_guarded(store: SqlitePlanTaskService):
