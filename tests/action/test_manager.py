...
def test_empty_name_raises_valueerror():
    ...
    for bad in (None, '', '   '):
        with pytest.raises(ValueError):
            mgr.register(bad, f)  # pyright: ignore[reportArgumentType]
        with pytest.raises(ValueError):
            mgr.unregister(bad)  # pyright: ignore[reportArgumentType]
        with pytest.raises(ValueError):
            mgr.execute(bad)  # pyright: ignore[reportArgumentType]


def test_non_callable_raises_typeerror():
    mgr = ActionManager()
    with pytest.raises(TypeError):
        mgr.register('ip.query', 'not-callable')  # pyright: ignore[reportArgumentType]
